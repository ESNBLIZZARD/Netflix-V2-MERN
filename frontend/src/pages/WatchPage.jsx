	...
	useEffect(() => {
		const getSimilarContent = async () => {
			try {
				...
				setSimilarContent(res?.data?.similarMovies || res?.data?.similar || res?.data?.similarTvShows );
			...
			<div className='mx-auto container px-4 py-8'>
				...
				<div className='flex flex-col md:flex-row items-center justify-between gap-12 max-w-6xl mx-auto'>
					<div className='mb-4 md:mb-0 flex-1'>
						...
						<p className='mt-4 text-lg'>{content?.overview || "No description available."}</p>
					</div>
					<div className='flex-1 flex justify-center'>
						<img
							...
							alt='Poster'
							className='max-h-[600px] rounded-md object-cover'
						/>
					</div>
				</div>
				...
				{similarContent.length > 0 && (
					<div className='mt-12 max-w-5xl mx-auto relative group'>
						...
						{/* Scroll Buttons */}
						<ChevronRight
							className='absolute top-1/2 -translate-y-1/2 right-2 w-8 h-8 opacity-0 group-hover:opacity-100 transition-all duration-300 cursor-pointer bg-red-600 text-white rounded-full z-10'
							onClick={scrollRight}
						/>
						<ChevronLeft
							className='absolute top-1/2 -translate-y-1/2 left-2 w-8 h-8 opacity-0 group-hover:opacity-100 transition-all duration-300 cursor-pointer bg-red-600 text-white rounded-full z-10'
							onClick={scrollLeft}
						/>
					</div>
				)}
